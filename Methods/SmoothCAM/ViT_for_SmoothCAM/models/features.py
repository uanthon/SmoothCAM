...
class FeatureInfo:
    ...
    def __init__(self, feature_info: List[Dict], out_indices: Tuple[int]):
        ...
        for fi in feature_info:
            ...
            assert 'module' in fi
        self.out_indices = out_indices  #保存输出的索引
        self.info = feature_info  #保存特征信息

    def from_other(self, out_indices: Tuple[int]):
        return FeatureInfo(deepcopy(self.info), out_indices)  #深度复制

    def get(self, key, idx=None):  #按键获取特征信息
    ...
    def channels(self, idx=None):  #返回特征通道数
        """ feature channels accessor
        """
        return self.get('num_chs', idx)

    def reduction(self, idx=None):  #返回特征缩减率
        """ feature reduction (output stride) accessor
        """
        return self.get('reduction', idx)

    def module_name(self, idx=None):  #返回特征模块名
        """ feature module name accessor
        """
        return self.get('module', idx)

    def __getitem__(self, item):  #使得 FeatureInfo 实例支持索引访问。
        return self.info[item]

    def __len__(self):  #使得 FeatureInfo 实例支持使用 len() 函数。
        return len(self.info)


class FeatureHooks:  #帮助设置和提取特征钩子
    """ Feature Hook Helper
    ...
    def _collect_output_hook(self, hook_id, *args):  #收集钩子的输出
        x = args[-1]  # tensor we want is last argument, output for fwd, input for fwd_pre
        if isinstance(x, tuple):
            x = x[0]  # unwrap input tuple
        self._feature_outputs[x.device][hook_id] = x

    def get_output(self, device) -> Dict[str, torch.tensor]:  #返回并清空指定设备的特征输出
        output = self._feature_outputs[device]
        self._feature_outputs[device] = OrderedDict()  # clear after reading
        return output


def _module_list(module, flatten_sequential=False):  #函数返回模块列表
...
def _get_feature_info(net, out_indices):  #函数获取并返回 FeatureInfo 实例
...
def _get_return_layers(feature_info, out_map):  #函数返回指定的输出层
...
class FeatureDictNet(nn.ModuleDict):
    ...
    def forward(self, x) -> Dict[str, torch.Tensor]:  #forward 方法调用 _collect 收集特征输出
